...
Forward custom environment variables into the Docker container when argsh delegates to Docker (e.g., `argsh test`, `argsh lint`). Any variable prefixed with `ARGSH_ENV_` is forwarded with the prefix stripped.
