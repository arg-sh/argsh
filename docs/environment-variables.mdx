...
```bash
ARGSH_TRACE=/tmp/trace.md argsh ./myscript.sh deploy --env prod
```

The resulting markdown file contains:

- **Header** with script name, date, arguments, and PID
- **Execution** section with indented function entry/exit events and elapsed times
- **Variables** collapsible sections showing parsed argument values after each `:args` call
- **Summary** with total steps, duration, and exit code

This is useful for debugging complex multi-subcommand scripts, understanding the call flow, and diagnosing argument parsing issues.
