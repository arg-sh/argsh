...
**Positional**
A field without a pipe `|` character. Matched by position on the command line rather than by flag name. A positional is **required** when its variable is uninitialized (`local name`) and **optional** when initialized with a default (`local name=""`). Array positionals consume all remaining arguments.
...
| `:!` | Required flag (must be provided; flags only — positional required/optional is controlled by variable initialization) | `env\|e:!` |
...
**`argsh-so`**
A self-contained executable with the native `.so` appended as a raw binary payload. Extracted at runtime via `tail` at a known byte offset. Shebang-only — cannot be sourced. See [Self-Contained Variant](/development/fundamentals/builtins#self-contained-variant-argsh-so).
