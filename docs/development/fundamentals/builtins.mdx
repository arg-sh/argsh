...
:::note
The `.so` is compiled for the current platform. Cross-compilation requires the appropriate Rust target and compatible bash headers. See [Compatibility](#compatibility) for glibc and Bash version requirements.
:::
