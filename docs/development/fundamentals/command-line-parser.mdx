...
The `:^` modifier marks a field as "inheritable" — it yields to non-`:^` duplicates with the same field name. This enables parent/child function patterns where a parent defines a flag and children inherit it via `"${args[@]}"` expansion.
...
**How it works:**
- When called from a parent (e.g. `lo secrets init --domain prod`): the parent's `args` array flows in via `"${args[@]:-}"`, the parent's `domain|` entry (non-`:^`) wins over the local `domain|:^`, and the parent's already-parsed `domain` value is inherited via bash dynamic scope
- When called standalone (`secrets init --domain prod`): no parent `args` exist, `"${args[@]:-}"` expands to nothing, the local `:^` entry is used as a normal field
- `local domain="${domain:-${DOMAIN_NAME:-}}"` inherits the parent's value if set, falls back to env var, then empty
