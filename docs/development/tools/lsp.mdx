...
:::tip
The same diagnostics engine is also available as a standalone CLI binary — `argsh-lint` — for CI pipelines and editor-agnostic integrations. See [How to Lint your scripts](../fundamentals/lint.mdx) for the command-line interface. Both the LSP server and the CLI share a single Rust crate so the set of checks stays in sync.
:::
...
:::tip
Set `ARGSH_DEBUG=1` to see debug trace output for import resolution and builtin loading. See [Environment Variables](/environment-variables) for the full reference.
:::
