...
With `set -e`, the left-hand side of `&&`/`||` is exempt from errexit — only the command after the **final** `&&` or `||` can trigger an exit. In the pattern above, the `[[ ]]` failure is safe, and the `{ ...; exit 1; }` handles the error explicitly.
