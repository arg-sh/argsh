...
# @set LOCK_FILE string Optional override for lockfile path
# @internal
argsh::lib::_write_lock_entry() {
  local _key="${1}" _ref="${2}" _digest="${3}"
  local _dir="${PATH_BASE:-.}"
  local _lock="${LOCK_FILE:-${_dir}/.argsh.lock}"
...
argsh::lib::add() {
  ...
  if (( _global )); then
    ...
    LOCK_FILE="${__ARGSH_GLOBAL_LIBS}/.argsh-global.lock" \
...
argsh::lib::update() {
  ...
  if (( _global )); then
    local _glock="${__ARGSH_GLOBAL_LIBS}/.argsh-global.lock"
    if [[ ! -f "${_glock}" ]]; then
      echo "argsh: no global lockfile found (${_glock})" >&2
      return 1
    fi
    # Parse global lockfile: re-add each lib at latest version
    local _entry_key="" _in_libs=0 _failed=0 _line
    while IFS= read -r _line || [[ -n "${_line}" ]]; do
      [[ -n "${_line}" && "${_line}" != \#* ]] || continue
      if [[ "${_line}" =~ ^libs:[[:space:]]*$ ]]; then _in_libs=1; continue; fi
      if (( _in_libs )) && [[ "${_line}" =~ ^[^[:space:]] ]]; then break; fi
      ...
      if [[ "${_line}" =~ ^[[:space:]][[:space:]]([^[:space:]:\"]+|\"[^\"]+\"):$ ]]; then
        _entry_key="${BASH_REMATCH[1]}"
        _entry_key="${_entry_key%\"}" ; _entry_key="${_entry_key#\"}"
        echo "Updating ${_entry_key}..."
        argsh::lib::add --global "${_entry_key}" || { echo "argsh: failed to update ${_entry_key}" >&2; _failed=1; }
      fi
    done < "${_glock}"
    return "${_failed}"
  fi
...
# @description Search available libraries from the OCI registry.
# Queries the GitHub API for releases from arg-sh/libs and groups them by
# library name, showing the latest version for each. An optional filter
# argument restricts output to libraries whose name contains the filter string.
# Uses GITHUB_TOKEN/GH_TOKEN when available to avoid rate limits.
# @arg $1 string Optional name filter (substring match)
# @internal
argsh::lib::search() {
  local _filter="${1:-}"

  # Build curl args — use auth token if available
  local -a _curl_args=(-fsSL)
  if [[ -n "${GITHUB_TOKEN:-${GH_TOKEN:-}}" ]]; then
    _curl_args+=(-H "Authorization: token ${GITHUB_TOKEN:-${GH_TOKEN}}")
  fi
  ...
  local _response
  _response="$(curl "${_curl_args[@]}" \
    "https://api.github.com/repos/arg-sh/libs/releases?per_page=100")" || {
    echo "argsh search: failed to query GitHub API" >&2
    return 1
  }

  if [[ -z "${_response}" ]]; then
    echo "argsh search: no releases found" >&2
    return 1
  fi

  # Parse tag_name entries: format is <name>/v<version>
  # Extract all name/version pairs, then keep only the latest (first seen) per name.
  local -A _latest=()
  local -a _order=()
  local _tag _name _version
  while IFS= read -r _tag; do
    [[ "${_tag}" == */* ]] || continue
    _name="${_tag%%/*}"
    _version="${_tag#*/v}"
    # Skip if name doesn't match filter
    if [[ -n "${_filter}" && "${_name}" != *"${_filter}"* ]]; then
      continue
    fi
    # Keep first occurrence (GitHub returns newest first)
    if [[ -z "${_latest["${_name}"]:-}" ]]; then
      _latest["${_name}"]="${_version}"
      _order+=("${_name}")
    fi
  done < <(grep -o '"tag_name": *"[^"]*"' <<< "${_response}" | sed 's/"tag_name": *"//; s/"$//')

  if [[ ${#_order[@]} -eq 0 ]]; then
    if [[ -n "${_filter}" ]]; then
      echo "No libraries found matching '${_filter}'"
    else
...
argsh::main() {
  local -a usage=(
    ...
    'search:-argsh::lib::search' "Search available libraries"
    '-'                          "Runtime"
