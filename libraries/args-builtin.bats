...
#
# Same tests as args.bats but with :usage and :args loaded as native builtins.
# The bash function implementations are replaced by the Rust loadable builtins.

BATS_LOAD="${BATS_TEST_FILENAME%/*}/args.sh"
PATH_FIXTURES="$(realpath "$(dirname "${BATS_TEST_FILENAME}")/fixtures/args")"
PATH_SNAPSHOTS="${PATH_FIXTURES}/snapshots-builtin"
load ../test/helper

# Source args.sh for all helpers, then replace :usage/:args with builtins
load_source
enable -f "${BATS_TEST_FILENAME%/*}/../builtin/target/release/libargsh_builtin.so" :usage :args
unset -f :usage :args
